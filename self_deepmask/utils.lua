...
function utils.filter_large_masks(areas,threshold)
   threshold = math.floor(threshold)
   print('Filter large masks')
   sorted_areas = areas:sort(1,true)
   sorted_areas, indices = torch.sort(areas,true)
   local keep = torch.le(sorted_areas,threshold)
   indices = indices[torch.le(sorted_areas,threshold)]
   return indices
end

function utils.crop_boxes(bboxes,crop_box, w , h)
    local pick = {}
    local pickIdx = 1
    local removed = 0
    for i=1,bboxes:size(1) do
      local bbox = bboxes[i]
      local x_center = math.floor(bbox[1]+bbox[3]/2.0)
      local y_center = math.floor(bbox[2]+bbox[4]/2.0)
      -- Check if center is inside crop box
      if x_center >= crop_box[1] and x_center <= (crop_box[1]+crop_box[3]) and y_center >= crop_box[2] and y_center <= (crop_box[2]+crop_box[4]) then
        -- Heuristic: remove boxes that touch image borders
        local padding = 10
        if bbox[1] <= padding or (bbox[1]+bbox[3]) >= (w-padding) or bbox[2] <= padding or (bbox[2]+bbox[4]) >= (h-padding) then
          removed = removed + 1
        else
          pick[pickIdx] = i
          pickIdx = pickIdx+1
    ...
    picks = torch.LongTensor(#pick)
    s = picks:storage()
    for i=1,#pick do
        picks[i] = pick[i]
    end
    print("Crop filter number of removed masks:",removed, "/",bboxes:size(1))
    return picks
end
