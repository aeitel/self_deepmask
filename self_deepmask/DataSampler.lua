...
function DataSampler:__init(config,split)
  ...
  -- augmentation
  --self.augHSV = {H=0.1, S=0.1, V=0.001}
  self.augHSV = {H=0.0, S=0.0, V=0.0}
  if self.augHSV.H > 0 then
    print("Use HSV augmentation",self.augHSV.H,self.augHSV.S,self.augHSV.V)
  end
  if self.augHSV.S > 0 then
    print("Use HSV augmentation",self.augHSV.H,self.augHSV.S,self.augHSV.V)
  end
  if self.augHSV.V > 0 then
    print("Use HSV augmentation",self.augHSV.H,self.augHSV.S,self.augHSV.V)
  end
...
function DataSampler:augmentHSV(im_rgb, augHSV)
    ...
    if augHSV.H >0 then
        ...
        --im_hsv[1] = im_hsv[1]+(1 + torch.normal(0, augHSV.H))
        local add_sample = torch.Tensor(1):fill(1)
        local h_add_coeff = torch.Tensor(1):fill(augHSV.H)
        add_sample:mul(torch.rand(1),2):csub(1)
        h_add_coeff:mul(add_sample[1])

        im_hsv[1] = im_hsv[1]+h_add_coeff[1]
        im_hsv[1]:clamp(0, 1)
    end
    if augHSV.S >0 then
        local pow_sample = torch.Tensor(1):fill(1)
        local mul_sample =  torch.Tensor(1):fill(1)
        local add_sample = torch.Tensor(1):fill(1)
        local s_pow_coeff = torch.Tensor(1):fill(2)
        local s_mul_coeff = torch.Tensor(1):fill(0.5)
        local s_add_coeff = torch.Tensor(1):fill(augHSV.S)
        pow_sample:mul(torch.rand(1),2):csub(1)
        mul_sample:mul(torch.rand(1),2):csub(1)
        add_sample:mul(torch.rand(1),2):csub(1)

        s_pow_coeff:mul(pow_sample[1])
        s_mul_coeff:mul(mul_sample[1])
        s_add_coeff:mul(add_sample[1])

        local pow_deform = torch.pow(torch.Tensor(1):fill(2), s_pow_coeff[1])
        local mul_deform = torch.pow(torch.Tensor(1):fill(2), s_mul_coeff[1])
        local aug = torch.pow(im_hsv[2], pow_deform[1])
        aug:mul(mul_deform[1])

        im_hsv[2] = aug+s_add_coeff[1]
        im_hsv[2]:clamp(0, 1)
    end
    if augHSV.V >0 then
        local pow_sample = torch.Tensor(1):fill(1)
        local mul_sample =  torch.Tensor(1):fill(1)
        local add_sample = torch.Tensor(1):fill(1)
        local v_pow_coeff = torch.Tensor(1):fill(2)
        local v_mul_coeff = torch.Tensor(1):fill(0.5)
        local v_add_coeff = torch.Tensor(1):fill(augHSV.V)
        pow_sample:mul(torch.rand(1),2):csub(1)
        mul_sample:mul(torch.rand(1),2):csub(1)
        add_sample:mul(torch.rand(1),2):csub(1)

        v_pow_coeff:mul(pow_sample[1])
        v_mul_coeff:mul(mul_sample[1])
        v_add_coeff:mul(add_sample[1])

        local pow_deform = torch.pow(torch.Tensor(1):fill(2), v_pow_coeff[1])
        local mul_deform = torch.pow(torch.Tensor(1):fill(2), v_mul_coeff[1])
        local aug = torch.pow(im_hsv[3], pow_deform[1])
        aug:mul(mul_deform[1])

        --im_hsv[3] = aug+v_add_coeff[1]
        --print(v_add_coeff[1])
        im_hsv[3] = im_hsv[3]+v_add_coeff[1]
        im_hsv[3]:clamp(0, 1)
    end
...
function DataSampler:get(headSampling)
  ...
  if torch.uniform() > .5 then
    if torch.uniform() > .5 then
      ...
      local rad_theta = torch.uniform() * 2 * math.pi
      input = image.rotate(input,rad_theta)
      if headSampling == 1 then label = image.rotate(label,rad_theta) end
    end
  end
...
function DataSampler:maskSampling()
  ...
  local cat,ann = torch.random(2)

  while not ann or ann.iscrowd == 1 or ann.area < 100 or ann.bbox[3] < 5
    or ann.bbox[4] < 5 or ann.bbox[1] < 10 or ann.bbox[2] < 10
    or (ann.bbox[1]+ann.bbox[3]) > (width-10) or (ann.bbox[2]+ann.bbox[4]) > (height-10) do
      -- Fix for case with just one object class
      local catId = 1
      --print("catId",catId)
      local annIds = self.coco:getAnnIds({catId=catId})
      --local annIds = self.coco:getAnnIds()
      --print("Annids",annIds)
      local annid = annIds[torch.random(annIds:size(1))]
      ann = self.coco:loadAnns(annid)[1]
      width = self.coco:loadImgs(ann.image_id)[1].width
      height = self.coco:loadImgs(ann.image_id)[1].height
  end
...
function DataSampler:scoreSampling(cat,imgId)
  ...
  local img = image.load(pathImg,3)
    if torch.uniform() > .5 then
      -- HSV augmentation
      img:div(255) -- HSV augmentation requires a [0:1] range
      img = self:augmentHSV(img, self.augHSV)
      img:mul(255)
      --if headSampling == 1 then
      --  filename = string.format('/tmp/%s.jpg', torch.uniform())
      --  print(filename)
